    ...
    for seed in range(args.num_seeds):
        ...
        for c in range(args.n_chains):
            ...
            model = initialize_from_glasso(
                ...
                all_pairs=all_pairs, eps=1e-3, target_d=args.target_d, rng=rng
            )
            evaluator = PosteriorEvaluator(model, X)
            stepper = AdaptiveMALAStepper()
...
base_args.N = N
base_args.num_seeds = 10
base_args.n_chains = 4
base_args.save_dir = "./results_lambda_sensitivity_erdos_renyi"

for target_d in TARGET_D_GRID:
    args_td = copy.deepcopy(base_args)
    args_td.target_d = target_d
    args_td.save_dir = os.path.join(base_args.save_dir, f"target_d_{target_d}")

    consolidated_path = os.path.join(
        args_td.save_dir, f"CONSOLIDATED_{GRAPH_TYPE}_N{N}_targetd{target_d}.json"
    )
    all_results = load_or_init_consolidated(args_td.save_dir, consolidated_path)

    summary = run_single_combination(args_td, graph_type=GRAPH_TYPE, ratio=RATIO)
    all_results = save_to_consolidated(
        all_results, f"{GRAPH_TYPE}_N{N}", target_d, summary, consolidated_path
    )

    print(f"\n### target_d={target_d} (lambda={N * target_d / 2:.0f}) done: "
          f"F1_mean={summary['metrics_summary']['f1_mean']:.4f}, "
          f"Frobenius_mean={summary['metrics_summary']['frobenius_error_mean']:.4f}\n")