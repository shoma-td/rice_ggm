...
class SparseFactorizedPrior:
    ...
    def _cholesky_update(L: np.ndarray, v: np.ndarray) -> np.ndarray:
        """Rank-1 Cholesky update: L L^T -> L L^T + v v^T, via Givens
        rotations on the augmented [L | v] system. Cost: O(N^2)."""
        L_new = L.copy()
        v_cur = v.copy()
        N = len(v)
        for k in range(N):
    ...
    def _cholesky_downdate(L: np.ndarray, v: np.ndarray) -> np.ndarray:
        """Rank-1 Cholesky downdate: L L^T -> L L^T - v v^T, via hyperbolic
        rotations. Raises ValueError if the result would not be PD (should
        not happen given the eps floor in K). Cost: O(N^2)."""
        L_new = L.copy()
        v_cur = v.copy()
        N = len(v)
        for k in range(N):
...
STEPS_BY_RATIO = {
    0.5: {"n_steps": 15000, "burn_in": 4000},
    2:   {"n_steps": 20000, "burn_in": 5000},
    5:   {"n_steps": 25000, "burn_in": 6000},
    10:  {"n_steps": 30000, "burn_in": 8000},
    20:  {"n_steps": 40000, "burn_in": 10000},
}
...
base_args.save_dir = "./results_grid"   
